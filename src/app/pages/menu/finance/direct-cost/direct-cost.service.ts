...
import { directCostList } from './data';
import { DecimalPipe } from '@angular/common';
import { debounceTime, delay, switchMap, tap } from 'rxjs/operators';
import { SortColumn, SortDirection } from './direct-cost-sortable.directive';
import { ApiService } from 'src/app/API/api.service';

interface SearchResult {
  countries: directCostModel[];
  total: number;
}
...
function sort(countries: directCostModel[], column: SortColumn, direction: string): directCostModel[] {
if (direction === '' || column === '') {
  return countries;
} else {
...
function matches(country: directCostModel, term: string, pipe: PipeTransform) {
  return country.MonthYear.toLowerCase().includes(term.toLowerCase())
    || country.TypeOfEmployeement.toLowerCase().includes(term.toLowerCase())
    || country.Department.toLowerCase().includes(term.toLowerCase())
    || country.ServiceLine.toLowerCase().includes(term.toLowerCase())
...
export class DirectCostService {
  ...
  private _countries$ = new BehaviorSubject<directCostModel[]>([]);
