...
import { indirectLabourCostList } from './data';
import { DecimalPipe } from '@angular/common';
import { debounceTime, delay, switchMap, tap } from 'rxjs/operators';
import { SortColumn, SortDirection } from './indirect-labour-cost-sortable.directive';
import { ApiService } from 'src/app/API/api.service';

interface SearchResult {
  countries: IndirectCostModel[];
  total: number;
}
...
function sort(countries: IndirectCostModel[], column: SortColumn, direction: string): IndirectCostModel[] {
if (direction === '' || column === '') {
  return countries;
} else {
...
function matches(country: IndirectCostModel, term: string, pipe: PipeTransform) {
  return country.MonthYear.toLowerCase().includes(term.toLowerCase())
    
    || country.Department.toLowerCase().includes(term.toLowerCase())
    
    || country.FC.toLowerCase().includes(term.toLowerCase())
    || country.FXRate.toLowerCase().includes(term.toLowerCase())
    || country.CostCTC.toLowerCase().includes(term.toLowerCase())
    || country.Branch.toLowerCase().includes(term.toLowerCase())
    || country.NoofManDays.toLowerCase().includes(term.toLowerCase())
...
export class IndirectLabourCostService {


  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _countries$ = new BehaviorSubject<IndirectCostModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe, private apiservice: ApiService) {
    ...
    this.apiservice.GetDataWithToken('indirectlabourcost/display').subscribe(
      (directcostList) => {
