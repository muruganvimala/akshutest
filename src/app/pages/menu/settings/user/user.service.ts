...
import { SortColumn, SortDirection } from './user-sortable.directive';
...
function sort(users: userModel[], column: SortColumn, direction: SortDirection): userModel[] {
  if (direction === '' || column === '') {
    return users;
  } else {
...
function matches(user: userModel, term: string, pipe: PipeTransform) {
...
export class UserService {
  ...
  private _countries$ = new BehaviorSubject<userModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    if (this.productStatus) {
      countries = countries.filter(country => country.firstname == this.productStatus);
    }
    else {
      countries = countries;
    }
