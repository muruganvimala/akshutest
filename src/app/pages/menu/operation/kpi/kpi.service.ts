...
import { SortColumn, SortDirection } from './kpi-sortable.directive';
...
function sort(countries: kpiModel[], column: SortColumn, direction: string): kpiModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: kpiModel, term: string, pipe: PipeTransform) {
    return country.Publisher.toLowerCase().includes(term.toLowerCase())
      || country.MonthYear.toLowerCase().includes(term.toLowerCase())
      || country.OverallPerformance.toLowerCase().includes(term.toLowerCase())
      || country.Schedule.toLowerCase().includes(term.toLowerCase())
      || country.Quality.toLowerCase().includes(term.toLowerCase())
      || country.Communication.toLowerCase().includes(term.toLowerCase())
      || country.CustomerSatisfaction.toLowerCase().includes(term.toLowerCase())
      || country.AccountManagement.toLowerCase().includes(term.toLowerCase())
      || country.RFT.toLowerCase().includes(term.toLowerCase())
      || country.PublicationSpeed.toLowerCase().includes(term.toLowerCase())
      || country.Feedback.toLowerCase().includes(term.toLowerCase())
      || country.AuthorSatisfaction.toLowerCase().includes(term.toLowerCase())
      ;
  }
  ...
  export class KpiService {
    ...
    private _countries$ = new BehaviorSubject<kpiModel[]>([]);
