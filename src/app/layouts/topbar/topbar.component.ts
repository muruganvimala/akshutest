...
*/
  ...
  */
  ...
  logout() {
    ...
    console.log('after logout local storage length is ' + sessionStorage.length);
  ...
  validateToken(): void {
    // Given expiration date string
    let expiresString: string | null = sessionStorage.getItem('expires');

    // Convert the expiration string to a Date object
    if (expiresString != null) {
        let expiresDate: Date = new Date(expiresString);
        ...
        if (expiresDate > currentDate) {
            ...
            const timer = setInterval(() => {
                ...
                if (content) {
                    ...
                    if (pElement) {
                        pElement.textContent = `Session Expired. Closing in ${remainingTime} seconds.`;
                    }
                }
                if (remainingTime <= 0) {
                    clearInterval(timer);
            ...
            Swal.fire({
                title: 'Warning',
                html: '<p>Session Expired. Closing in 5 seconds.</p>',
