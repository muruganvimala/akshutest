...
export class SidebarComponent  {
  ...
  constructor(private router: Router, public translate: TranslateService,private api: ApiService) {
    translate.setDefaultLang('en');
  }

  ngOnInit(): void {
    if (sessionStorage.length === 0) {
      console.log('routing path /account/login');
    ...
    if (userRole) {
        ...
        this.api.GetDataWithToken(`Rolemaster/getMenuByRole/${userRole}`).subscribe((data: any[]) => {
          this.menuItems = data;
