...
import { SortColumn, SortDirection } from './role-sortable.directive';
...
function sort(users: roleModel[], column: SortColumn, direction: SortDirection): roleModel[] {
  if (direction === '' || column === '') {
    return users;
  } else {
...
function matches(user: roleModel, term: string, pipe: PipeTransform) {
  return user.roleMaster.roleName.toLowerCase().includes(term.toLowerCase())
    || user.roleMaster.description.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class roleService {
  ...
  private _countries$ = new BehaviorSubject<roleModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe, private apiService: ApiService) {
    ...
    this.apiService.GetDataWithToken('Usermaster/Display').subscribe(
      (list) => {
        // Assign API response to products
  ...
  get product() { return this.userList; }
  ...
  private _search(): Observable<SearchResult> {
    ...
    if (this.productStatus) {
      countries = countries.filter(country => country.roleName == this.productStatus);
    }
    else {
      countries = countries;
    }
