...
import { LoadingSpinnerComponent } from './../../loading-spinner/loading-spinner.component'; // Adjust the path as per your project structure
...
export class KpiComponent implements OnInit, AfterViewInit {
  ...
  constructor(private formBuilder: UntypedFormBuilder, private apiService: ApiService, private datePipe: DatePipe, private cdr: ChangeDetectorRef, private renderer: Renderer2) {
   ...
   */
    ...
    console.log('isprogressLoading ' + this.isprogressLoading);
    this.appRole = sessionStorage.getItem('userRole');
  ...
  getRoleTableData(): void {
    this.apiService.GetDataWithToken(`Rolemaster/getUserAccess?userrole=${this.appRole}&parentmenuid=5&childmenuid=52`).subscribe(
      (response) => {
  ...
  filteredItems(): any[] {
    ...
    if (pageSizeNumber === -1) {
      ...
      if (this.searchTerm) {
        ...
        if (this.selectedPublisher != "" || this.selectedYear != "" || this.selectedMonthYear != "" || this.selectedEndDate != "" || this.selectedStartDate != "") {
  ...
  resetFilter() {
    $('#PubFilter').val('');
    $('#yearFilter').val('Select');
    $('#filterMonthYear').val('Select');
    $('#startDate').val('Select');
    $('#endDate').val('Select');
  ...
  */
  ...
  exportToExcelAll() {
    ...
    //       SheetNames: ['data'],
    //     };
    //     XLSX.writeFile(workbook, 'consignment.xlsx');
    //   },
    //   (error: any) => {
  ...
  exportToCsvAll() {
    ...
    //     const link = document.createElement('a');
    //     link.setAttribute('href', encodedUri);
    //     link.setAttribute('download', 'consignment.csv');
    //     document.body.appendChild(link);
    //     link.click();
    //   },
    //   (error: any) => {
